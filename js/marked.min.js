...
`),
							...
							((r[r.length - 1] = y),
								(s =
									...
									y.raw),
								(i =
									i.substring(0, i.length - T.text.length) +
									y.text));
							break;
						} else if (g?.type === 'list') {
							let T = g,
...
`),
							...
							((r[r.length - 1] = y),
								(s =
									s.substring(0, s.length - g.raw.length) +
									y.raw),
								(i =
									i.substring(0, i.length - T.raw.length) +
									y.raw),
...
`,
								...
								if (
									...
									break;
								if (
									C.search(this.rules.other.nonSpaceChar) >=
										f ||
									!g.trim()
								)
									u +=
...
`),
					...
					for (let c = 0; c < i.items.length; c++)
						if (
							...
							let p = i.items[c].tokens.filter(
									(d) => d.type === 'space',
								),
								u =
									p.length > 0 &&
									p.some((d) =>
										this.rules.other.anyLine.test(d.raw),
									);
							i.loose = u;
						}
					if (i.loose)
...
` + s.text),
			...
			inlineTokens(e, t = []) {
				...
				for (
					;
					(s = this.tokenizer.rules.inline.blockSkip.exec(n)) != null;
				)
					n =
						n.slice(0, s.index) +
						'[' +
		...
		var E = class {
				...
				return (
					e.forEach((n) => {
						...
						if (
							...
							let i =
								this.defaults.renderer || new $(this.defaults);
							for (let r in n.renderer) {
								if (!(r in i))
									throw new Error(
										`renderer '${r}' does not exist`,
									);
						...
						if (n.tokenizer) {
							let i =
								this.defaults.tokenizer || new S(this.defaults);
							for (let r in n.tokenizer) {
								if (!(r in i))
									throw new Error(
										`tokenizer '${r}' does not exist`,
									);
			...
			parseMarkdown(e) {
				return (n, s) => {
					...
					if (r.async)
						return Promise.resolve(
							r.hooks ? r.hooks.preprocess(n) : n,
						)
							.then((p) => a(p, r))
							.then((p) =>
								r.hooks ? r.hooks.processAllTokens(p) : p,
							)
							.then((p) =>
								r.walkTokens
									? Promise.all(
											this.walkTokens(p, r.walkTokens),
										).then(() => p)
									: p,
							)
							.then((p) => c(p, r))
